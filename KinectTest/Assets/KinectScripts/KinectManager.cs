...
//				break;
	...
	void Update()
	{
		if(KinectInitialized)
		{
			...
			if(ComputeUserMap)
			{
				float tempDepth = Time.realtimeSinceStartup;
				...
				{
					float difD = Time.realtimeSinceStartup - tempDepth;
					Debug.LogFormat("Profundidad: {0}", difD);
					UpdateUserMap();//LINEA DE CODIGO A MEDIR------------------------------------------------------------------------------------------------------------------------------
				}
			}
			
			if(ComputeColorMap)
			{
				float tempColor = Time.realtimeSinceStartup;
				if(colorStreamHandle != IntPtr.Zero &&
					KinectWrapper.PollColor(colorStreamHandle, ref usersColorMap, ref colorImage))
				{
					float difC = Time.realtimeSinceStartup - tempColor;
					Debug.LogFormat("Color: {0}", difC);
					UpdateColorMap();//LINEA DE CODIGO A MEDIR----------------------------------------------------------------------------------------------------------------------------
				}
			}
							...
							gestureData.progress >= 0.5f)
						{
							if(GetGestureProgress(gestureData.userId, KinectGestures.Gestures.Click) < 0.3f)
							{
								if(HandCursor1 != null)
								{
									...
									if(HandCursor1 == null)						///////////////////////////////////////////////////////////////////////////////////////////////.guiTexture.
									{
